...
const LessonCreateDialog = ({ onSuccess }: { onSuccess?: () => void }) => {
  const { getDialog, closeDialog } = dialogStore(),
    dialog = getDialog('lesson_create');

  const create = useInsertMutation(supabase.from('lessons'), ['id']);
  ...
  const form = useFormik<Create>({
    ...
    onSubmit: async (val) => {
      const toastId = toast.loading('Menambahkan kurikulum...');
